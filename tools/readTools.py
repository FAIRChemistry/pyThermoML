...
def __addProperties__(pureOrMixtureData, exp) -> list:
    ...
    for prop in list(pureOrMixtureData):
        elem = prop.findall(".//{http://www.iupac.org/namespaces/ThermoML}ePropName")
        for ePropName in elem:
            propIDs.append(exp.addProperty(__propertyTypeChecker__(ePropName.text, prop)))
    return propIDs
...
def __addVariables__(pureOrMixtureData, exp) -> list:
    ...
    for var in list(pureOrMixtureData):
        varType = var.findall(".//{http://www.iupac.org/namespaces/ThermoML}VariableType")
        for varEnum in list(varType):
            for varName in list(varEnum):
                varIDs.append(exp.addVariable(__variableTypeChecker__(varName.text, var)))
...
def __addMeasurements__(pureOrMixtureData, exp, propIDs, varIDs):
    ...
    for point in pureOrMixtureData.findall(".//{http://www.iupac.org/namespaces/ThermoML}NumValues"):
        ...
        exp.addMeasurements(Measurement(str(measnumb), values, pureOrMixtureData=exp))
        measnumb+=1
...
def __addPureOrMixtureData__(dataReport, root, compIDs):
    for pureOrMixtureData in root.iter('{http://www.iupac.org/namespaces/ThermoML}PureOrMixtureData'):
        ...
        exp = PureOrMixtureData(get(pureOrMixtureData, 'nPureOrMixtureDataNumber'), "", *compIDs)
...
def readThermo(path) -> DataReport:
    ...
    dataReport = DataReport(get(getChild(root, 'Citation'), 'sTitle'), get(getChild(root, 'Citation'), 'sDOI'), *__getAuthors__(getChild(root, 'Citation')))
